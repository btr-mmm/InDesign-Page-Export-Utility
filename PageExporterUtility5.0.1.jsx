...
for (currentDoc = 0; currentDoc < peuINFO.numDocsToExport; currentDoc++) {
  ...
  tempENTRY.singlePage =
    tempENTRY.theDoc.documentPreferences.pagesPerDocument == 1 ? true : false;
  tempENTRY.getOut = true;
  var baseName = tempENTRY.theDoc.name.split(".ind")[0];

  // Display the dialog box and loop for correct info
  if (
    (!commonBatchINFO.infoLoaded && peuINFO.batchSameForAll) ||
    ...
    do {
      ...
      var mainDialog = createMainDialog(
        tempENTRY.theDoc.name,
        currentDoc + 1,
        peuINFO.numDocsToExport
      );
      ...
      // Export format type
      tempENTRY.formatType =
        formatTypeRB.selectedButton + peuINFO.adjustForNoPS;
      ...
      if (peuINFO.pageNamePlacement == 2) {
        ...
        if (temp == -1) {
          //Token isn't there
          alert(
            "There is no page item token (<#>) in the base name. Please add one or change the page naming placement preference."
          );
          tempENTRY.getOut = false;
          pseudoSingleton--; // Allow prefs to be accessed again, but just once
          continue;
        } else pseudoSingleton++;
      } // Try to remove any <#>s as a precaution
      else tempENTRY.baseName = tempENTRY.baseName.replace(/<#>/g, "");
      ...
      if (currentDoc < 1) {
        ...
        if (peuINFO.numDocsToExport > 1) {
          ...
          if (!peuINFO.batchON) peuINFO.numDocsToExport = 1;
        }
      }
      ...
      if (peuINFO.doSpreadsON && tempENTRY.nameConvType == 1) {
        alert(
          "Spreads cannot be used with the 'Add \".L\"' option.\nThis combination is not supported. (1.1)"
        );
        ...
        alert(
          "Spreads cannot be used with the 'Numeric Override' option.\nThis combination is not supported. (1.2)"
        );
        tempENTRY.nameConvType = 0;
        tempENTRY.getOut = false;
        continue;
      ...
      if (peuINFO.doOneFile && tempENTRY.formatType > 1) {
        alert(
          "The 'Send Entire File At Once' option can only be used with PostScript or PDF formats. (1.3)"
        );
        tempENTRY.getOut = false;
        continue;
      }
      ...
      if (peuINFO.doBatch && peuINFO.batchSameForAll && startEndPgs != "All") {
        alert(
          "The 'Set For All Batch Jobs' option can only be used with a Page Range of 'All'. Page Range has been reset to 'All'. (1.4)"
        );
        startEndPgs = "All";
      }
      ...
      var tempPageCount = 0;
      if (tempENTRY.doOneFile) tempPageCount = 1;
      else {
    ...
    tempENTRY.useTag = usePgInfoTag(
      tempENTRY.theDoc.viewPreferences.horizontalMeasurementUnits,
      tempENTRY.theDoc.documentPreferences.pageWidth
    );
    ...
    switch (tempENTRY.formatType) {
      ...
      case 2: // EPS Formatting
        tempENTRY.epsINFO = getEPSoptions(
          tempENTRY.theDoc.name.split(".ind")[0]
        );
      ...
      case 3: // JPEG Formatting
        tempENTRY.jpegINFO = getJPEGoptions(
          tempENTRY.theDoc.name.split(".ind")[0]
        );
        break;
    }
    ...
    if (peuINFO.defaultDir == 0) {
      tempENTRY.outDir = getDirectory(
        "Please select the output directory:",
        peuINFO.startingDirectory
      );
      if (tempENTRY.outDir != null) tempENTRY.outDir += "/";
      else byeBye("Exporting has been canceled by user.", peuINFO.sayCancel);
    }
    ...
    // Get the base name for other batch jobs
    do {
      tempENTRY.getOut = true;
      var nameDialog = app.dialogs.add({
        name:
          VERSION_NAME +
          ': Base Name for "' +
          tempENTRY.theDoc.name +
      ...
      with (nameDialog) {
        with (dialogColumns.add()) {
          with (dialogRows.add()) {
            staticTexts.add({
              staticLabel:
                'Enter the Base Name for "' + tempENTRY.theDoc.name + '"',
            });
            var newBaseName = textEditboxes.add({
              editContents: baseName,
              minWidth: 135,
            });
          }
          with (dialogRows.add())
      ...
      if (!nameDialog.show()) {
        ...
        tempENTRY.baseName = removeColons(
          removeSpaces(newBaseName.editContents)
        );
        nameDialog.destroy();
        // Determine if page replacement token exists when the page token option is used
        if (peuINFO.pageNamePlacement == 2) {
          ...
          if (temp == -1) {
            //Token isn't there
            alert(
              "There is no page item token (<#>) in the base name. Please add one or click cancel in the next dialog box."
            );
            tempENTRY.getOut = false;
          }
        } // Try to remove any <#>s as a precaution
  ...
  if (peuINFO.layersON) {
    ...
    if (tempENTRY.layerINFO == null)
      // Only one layer, turn it off for this doc
      tempENTRY.layersON = false;
    else tempENTRY.layersON = true;
  }
  ...
  if (peuINFO.doProgressBar && tempENTRY.layersON) {
    ...
    for (i = 0; i < tempENTRY.layerINFO.verControls.length; i++)
      if (tempENTRY.layerINFO.verControls[i] == 1) temp++;
    if (!peuINFO.baseLaersAsVersion) temp--;
  }
...
for (currentDoc = 0; currentDoc < printINFO.length; currentDoc++) {
  ...
  if (peuINFO.doProgressBar) {
    ...
    if (progCancel) byeBye("User canceled export.", peuINFO.sayCancel);
  }
  ...
  if (currentINFO.doOneFile || currentINFO.singlePage) {
    ...
    if (currentINFO.layersON) {
      ...
      for (v = 0; v < versionControls.length; v++) {
        if (!versionControls[v]) continue;
        if (lastVersion != -1)
          // Turn the last layer back off
          theLayers[lastVersion].visible = false;
        lastVersion = v;
        theLayers[v].visible = true;
        currentINFO.outfileName = addPartToName(
          currentINFO.baseName,
          theLayers[v].name,
        ...
        if (peuINFO.doProgressBar) advanceBar();
      }
      ...
      if (peuINFO.baseLaersAsVersion) {
        ...
        for (v = 0; v < currentINFO.layerINFO.baseControls.length; v++) {
          if (currentINFO.layerINFO.baseControls[v])
            // its a base layer, keep track of last base version layer number
            lastVersion = v;
          else theLayers[v].visible = false;
        }
        if (!lastVersion == -1) {
          // Only export if there was a base version
          currentINFO.outfileName = addPartToName(
            currentINFO.baseName,
            theLayers[lastVersion].name,
            peuINFO.layerBeforeON
          );
      ...
      // No layer versioning, just export
    ...
    if (!peuINFO.batchON) byeBye("Done exporting as a single file.", true);
  } else {
    // Do single pages/spreads
    if (!currentINFO.hasNonContig)
      ...
      for (ii = 0; ii < currentINFO.nonContigPgs.length; ii++) {
        ...
        } // The non-contiguous page is a single page
        else outputPages(temp, temp, currentINFO);
      }
    }
  }
...
function outputPages(pgStart, pgEnd, currentINFO) {
  ...
  if (currentINFO.layersON) {
    ...
    if (peuINFO.baseLaersAsVersion) numVersions++;
  } else numVersions = 1;
  for (v = 0; v < numVersions; v++) {
    if (currentINFO.layersON) {
      if (v == numVersions - 1 && peuINFO.baseLaersAsVersion) {
        ...
        for (slbm = 0; slbm < baseControls.length; slbm++) {
          if (baseControls[slbm])
            // its a base layer, use its name for page name
            currentLayer = slbm;
          else theLayers[slbm].visible = false;
        }
        // Check if there was no base layer at all
        if (currentLayer == -1) layerName = "**NO_BASE**";
        else layerName = theLayers[currentLayer].name;
      } else {
        if (!versionControls[v]) continue;
        if (lastVersion != -1)
          // Turn the last layer back off
    ...
    if (currentINFO.nameConvType == 4) {
      ...
      currentPage = getPageOffset(
        pgStart,
        currentINFO.pageNameArray,
        currentINFO.pageRangeArray
      );
      numericallyLastPage = getPageOffset(
        pgEnd,
        currentINFO.pageNameArray,
        currentINFO.pageRangeArray
      );
    ...
    if (layerName != "**NO_BASE**") {
      do {
        currentINFO.outfileName = getPageName(
          currentPage,
          layerName,
          currentINFO
        );
        if (currentINFO.doSpreadsON) {
          pgRange =
            currentINFO.pageRangeArray[
              getPageOffset(
                currentINFO.theDoc.spreads[currentPage].pages[0].name,
                currentINFO.pageNameArray,
                currentINFO.pageRangeArray
              )
            ];
        ...
        else pgRange = currentINFO.pageRangeArray[currentPage];
...
function exportPage(currentINFO, pgRange) {
  ...
  switch (currentINFO.formatType) {
    ...
    case 1: // PDF Formatting
      ...
      currentINFO.theDoc.exportFile(
        ExportFormat.pdfType,
        new File(outFile + ".pdf"),
        false,
        currentINFO.pdfPRESET
      );
      break;
    case 2: // EPS Formatting
    ...
    case 3: // JPEG Formatting
      if (pgRange == PageRange.allPages) {
        ...
        app.jpegExportPreferences.jpegExportRange =
          ExportRangeOrAllPages.exportRange;
        app.jpegExportPreferences.pageString = pgRange;
      }
      currentINFO.theDoc.exportFile(
        ExportFormat.jpg,
        new File(outFile + ".jpg"),
        false
      );
      break;
  }
...
function getPageName(currentPage, layerName, currentINFO) {
  var pgRename = "";
  if (currentINFO.doSpreadsON)
    currentINFO["currentSpread"] =
      currentINFO.theDoc.spreads[currentPage].pages;
  switch (currentINFO.nameConvType) {
    ...
    case 1: // Add ".L" to the page name
      ...
      break;
    case 0:
    case 4: // As is or Numeric Override
        ...
        for (j = 0; j < currentINFO.currentSpread.length; j++) {
          if (currentINFO.currentSpread[j].appliedSection.includeSectionPrefix)
            var tempPage =
              currentINFO.pageRangeArray[
                getPageOffset(
                  currentINFO.currentSpread[j].name,
                  currentINFO.pageNameArray,
                  currentINFO.pageRangeArray
                )
              ];
          ...
          if (
            !isNaN(tempPageNum) &&
            ("" + tempPage).length == ("" + tempPageNum).length
          ) {
            if (peuINFO.addZeroON)
              tempPage = addLeadingZero(
                tempPageNum,
                currentINFO.theDoc.pages.length
              );
            if (peuINFO.addPon) tempPage = "P" + tempPage;
          }
          pgRename =
            j == 0
              ? tempPage
              : pgRename + peuINFO.charList[peuINFO.seperatorChar] + tempPage;
        }
      } else {
        // Create a new name for an individual page
        if (currentINFO.nameConvType == 4) pgRename = currentPage;
        else pgRename = currentINFO.pageNameArray[currentPage];
        if (
          !isNaN(parseInt(pgRename, 10)) &&
          ("" + pgRename).length == ("" + parseInt(pgRename, 10)).length
        ) {
          if (peuINFO.addZeroON)
            pgRename = addLeadingZero(
              pgRename,
              currentINFO.theDoc.pages.length
            );
          if (peuINFO.addPon) pgRename = "P" + pgRename;
        }
      }
      break;
  }
...
function addPartToName(theBase, addThis, addBefore) {
  ...
  return addBefore
    ? addThis + peuINFO.charList[peuINFO.seperatorChar] + theBase
    : theBase + peuINFO.charList[peuINFO.seperatorChar] + addThis;
}
...
function getPageOffset(pgToFind, pageNameArray, pageRangeArray) {
  ...
  for (offset = 0; offset < pageRangeArray.length; offset++) {
    if (
      ("" + pgToFind).toLowerCase() ==
        ("" + pageNameArray[offset]).toLowerCase() ||
      ("" + pgToFind).toLowerCase() ==
        ("" + pageRangeArray[offset]).toLowerCase()
    )
      return offset;
  }
  return -1;
...
function addLeadingZero(tempPageNum, pageCount) {
  if (peuINFO.zeroPadding == 0) {
    // Normal padding
    if (
      (tempPageNum < 10 && pageCount < 100) ||
      (tempPageNum > 9 && pageCount > 99 && tempPageNum < 100)
    )
      return addSingleZero(tempPageNum);
    else if (tempPageNum < 10 && pageCount > 99)
      return addDoubleZero(tempPageNum);
    else return "" + tempPageNum;
  } else if (peuINFO.zeroPadding == 1) {
    // Pad to 2 digits
    if (tempPageNum < 10) return addSingleZero(tempPageNum);
    else return "" + tempPageNum;
  } else {
    // Pad to 3 digits
    if (tempPageNum < 10) return addDoubleZero(tempPageNum);
    else if (tempPageNum < 100) return addSingleZero(tempPageNum);
    else return "" + tempPageNum;
  }
}
...
function recordPgNames(tempENTRY) {
  ...
  for (i = 0; i < tempENTRY.theDoc.documentPreferences.pagesPerDocument; i++) {
    ...
    tempENTRY.pageRangeArray[i] = aPage.appliedSection.includeSectionPrefix
      ? aPage.name
      : aPage.appliedSection.name + aPage.name;
  }
  return tempENTRY;
}
...
function getPSoptions(docName) {
  ...
  do {
    tempGetOut = true;
    PSdlog = app.dialogs.add({
      name: 'PostScript Options for "' + docName + '"',
      canCancel: true,
    });
    with (PSdlog)
      with (dialogColumns.add()) {
        ...
        printPreset = dropdowns.add({
          stringList: peuINFO.psPrinterNames,
          minWidth: 236,
          selectedIndex: peuINFO.defaultPrintPreset,
        });
        with (dialogRows.add())
          staticTexts.add({
            staticLabel:
              "Override PS Page Size (" +
              peuINFO.measureLableArray[peuINFO.measurementUnits] +
              ")",
          });
        ...
        with (dialogRows.add()) {
          ...
          changeAddPSextention = dropdowns.add({
            stringList: ["No", "Yes"],
            selectedIndex: peuINFO.addPSextention,
          });
        }
      }
    if (PSdlog.show()) {
      ...
      if (isNaN(pgHeight) || isNaN(pgWidth) || pgHeight < 0 || pgWidth < 0) {
        alert(
          "Both page height and width must be numeric and greater than zero (3.1)."
        );
        pgHeight = "0";
        pgWidth = "0";
        tempGetOut = false;
      ...
      if (pgHeight > 0 && pgWidth > 0)
        // User changed size, use the new size
        psOptions.ignore = false;

      psOptions["height"] =
        pgHeight + peuINFO.measureUnitArray[peuINFO.measurementUnits];
      psOptions["width"] =
        pgWidth + peuINFO.measureUnitArray[peuINFO.measurementUnits];
...
function setPSoptions(theINFO) {
  with (currentINFO.theDoc.printPreferences) {
    ...
    if (
      ...
      colorOutput != ColorOutputModes.inripSeparations
    )
      printBlankPages = true;
    if (theINFO.useTag) pageInformationMarks = true;
    else pageInformationMarks = false;
    if (!theINFO.psINFO.ignore) {
      try {
        ...
        alert(
          "The current PPD doesn't support custom page sizes. The page size from the Print Preset will be used (3.2)."
        );
      }
    }
  }
...
function getPDFoptions(theINFO) {
  var PDFdlog = app.dialogs.add({
    name: 'PDF Options for "' + theINFO.theDoc.name.split(".ind")[0] + '"',
    canCancel: true,
  });
  var temp = new Array();
  ...
  if (peuINFO.defaultPDFpreset > temp.length - 1) peuINFO.defaultPDFpreset = 0;
  with (PDFdlog)
    with (dialogColumns.add()) {
      with (dialogRows.add())
        staticTexts.add({ staticLabel: "PDF Export Preset:" });
      pdfPresets = dropdowns.add({
        stringList: temp,
        minWidth: 50,
        selectedIndex: peuINFO.defaultPDFpreset,
      });
    }
  if (PDFdlog.show()) {
    ...
    try {
      ...
      temp.pageInformationMarks =
        theINFO.useTag && temp.cropMarks ? true : false;
    } catch (e) {
      /*ignore it*/
    }
    PDFdlog.destroy();
...
function getJPEGoptions(docName) {
  var temp = new Array();
  var JPEGdlog = app.dialogs.add({
    name: 'JPEG Options for "' + docName + '"',
    canCancel: true,
  });
  with (JPEGdlog)
    with (dialogColumns.add()) {
      with (dialogRows.add()) staticTexts.add({ staticLabel: "Quality:" });
      JPEGquality = dropdowns.add({
        stringList: new Array("Low", "Medium", "High", "Maximum"),
        minWidth: 50,
        selectedIndex: peuINFO.defaultJPEGquality,
      });
      with (dialogRows.add())
        staticTexts.add({ staticLabel: "Encoding Type:" });
      JPEGrender = dropdowns.add({
        stringList: ["Baseline", "Progressive"],
        minWidth: 50,
        selectedIndex: peuINFO.defaultJPEGrender,
      });
    }
  if (JPEGdlog.show()) {
...
function setJPEGoptions(theINFO) {
  with (app.jpegExportPreferences) {
    ...
    jpegRenderingStyle = theINFO.renderType
      ? JPEGOptionsFormat.baselineEncoding
      : JPEGOptionsFormat.progressiveEncoding;
  }
}
...
function getEPSoptions(docName) {
  ...
  var epsDialog = app.dialogs.add({
    name: 'EPS Options for "' + docName + '"',
    canCancel: true,
  });
      ...
      with (dialogRows.add())
        with (borderPanels.add())
          with (dialogColumns.add()) {
            ...
            changeFlattenerPreset = dropdowns.add({
              stringList: peuINFO.flattenerNames,
              minWidth: 180,
              selectedIndex: peuINFO.defaultFlattenerPreset,
            });
            with (dialogRows.add())
              changeIgnoreOverride = checkboxControls.add({
                staticLabel: "Ignore Overrides",
                checkedState: peuINFO.ignoreON,
              });
            with (dialogRows.add())
              staticTexts.add({ staticLabel: "Preview Type:" });
            changePreviewPreset = dropdowns.add({
              stringList: peuINFO.previewTypes,
              minWidth: 180,
              selectedIndex: peuINFO.defaultPreview,
            });
            with (dialogRows.add()) {
              staticTexts.add({ staticLabel: "Bleed:" });
              changeBleedVal = realEditboxes.add({
                editValue: peuINFO.bleed,
                minWidth: 60,
              });
              staticTexts.add({
                staticLabel:
            ...
            with (dialogRows.add()) {
              ...
              changeOpiEPS = checkboxControls.add({
                staticLabel: "EPS",
                checkedState: peuINFO.epsON,
              });
              changeOpiPDF = checkboxControls.add({
                staticLabel: "PDF",
                checkedState: peuINFO.pdfON,
              });
              changeOpiBitmap = checkboxControls.add({
                staticLabel: "Bitmapped",
                checkedState: peuINFO.bitmapON,
              });
            }
          }
    }
      ...
      with (borderPanels.add()) {
        with (dialogColumns.add()) {
          with (dialogRows.add()) {
            ...
            var changePSlevel = dropdowns.add({
              stringList: ["2", "3"],
              minWidth: 75,
              selectedIndex: peuINFO.psLevel,
            });
          }
          with (dialogRows.add()) {
            staticTexts.add({ staticLabel: "Color mode:" });
            if (peuINFO.csVersion == 3)
              var changeColorMode = dropdowns.add({
                stringList: ["Unchanged", "Grayscale", "RGB", "CMYK"],
                minWidth: 100,
                selectedIndex: peuINFO.colorType,
              });
            else
              var changeColorMode = dropdowns.add({
                stringList: [
          ...
          with (dialogRows.add()) {
            ...
            var changeFontEmbedding = dropdowns.add({
              stringList: ["None", "Complete", "Subset"],
              minWidth: 100,
              selectedIndex: peuINFO.fontEmbed,
            });
          }
          with (dialogRows.add()) {
            staticTexts.add({ staticLabel: "Type of data to send:" });
            var changeDataToSend = dropdowns.add({
              stringList: ["All", "Proxy"],
              minWidth: 50,
              selectedIndex: peuINFO.dataSent,
            });
          }
          with (dialogRows.add()) {
            staticTexts.add({ staticLabel: "Data type:" });
            var changeDataType = dropdowns.add({
              stringList: ["Binary", "ASCII"],
              minWidth: 50,
              selectedIndex: peuINFO.dataType,
            });
          }
          with (dialogRows.add()) {
            staticTexts.add({ staticLabel: "Perform OPI replacement:" });
            var changeOPIreplace = dropdowns.add({
              stringList: ["No", "Yes"],
              minWidth: 50,
              selectedIndex: peuINFO.opiReplacement,
            });
          }
        }
      }
  ...
  do {
    ...
    if (epsDialog.show()) {
      ...
      if (isNaN(peuINFO.bleed)) {
        ...
        try {
          app.epsExportPreferences.bleedBottom =
            "" +
            peuINFO.bleed +
            peuINFO.measureUnitArray[peuINFO.measurementUnits];
        } catch (Exception) {
          alert(
            "The bleed value must be less than one of the following: 6 in | 152.4 mm | 432 pt | 33c9.384"
          );
          getOut = false;
          peuINFO.bleed = oldBleed;
        }
...
function setEPSoptions(theINFO) {
  with (app.epsExportPreferences) {
    appliedFlattererPreset =
      peuINFO.flattenerNames[theINFO.defaultFlattenerPreset];
    bleedBottom =
      "" + theINFO.bleed + peuINFO.measureUnitArray[peuINFO.measurementUnits];
...
function createMainDialog(docName, thisNum, endNum) {
  var theDialog = app.dialogs.add({
    name:
      ...
      docName +
      '"' +
      (endNum == 1 ? "" : " (" + thisNum + " of " + endNum + " documents)"),
    canCancel: true,
  });
  with (theDialog) {
    // Left Column
        ...
        with (dialogColumns.add()) {
          ...
          with (namingConvention) {
            radiobuttonControls.add({
              staticLabel: "As Is",
              checkedState: peuINFO.nameConvType == 0,
            });
            radiobuttonControls.add({
              staticLabel: 'Add ".L"',
              checkedState: peuINFO.nameConvType == 1,
            });
            radiobuttonControls.add({
              staticLabel: 'Odd/Even = ".F/.B"',
              checkedState: peuINFO.nameConvType == 2,
            });
            radiobuttonControls.add({
              staticLabel: 'Odd/Even = "LA.F/LA.B"',
              checkedState: peuINFO.nameConvType == 3,
            });
            radiobuttonControls.add({
              staticLabel: "Numeric Override",
              checkedState: peuINFO.nameConvType == 4,
            });
          }
          with (dialogRows.add()) staticTexts.add({ staticLabel: "" });
          with (dialogRows.add()) {
            staticTexts.add({ staticLabel: "Base Name" });
            newBaseName = textEditboxes.add({
              editContents: baseName,
              minWidth: 100,
            });
          }
    ...
    with (dialogColumns.add()) {
      ...
      with (borderPanels.add()) {
        with (dialogColumns.add()) {
          with (dialogRows.add()) {
            ...
            startEndPgs = textEditboxes.add({
              editContents: peuINFO.editStartEndPgs,
              minWidth: 103,
            });
          }
          with (dialogRows.add())
            doSpreads = checkboxControls.add({
              staticLabel: "As Spreads",
              checkedState: peuINFO.doSpreadsON,
            });
          with (dialogRows.add())
            oneFile = checkboxControls.add({
              staticLabel: "Send Entire File At Once (PS+PDF only)",
              checkedState: peuINFO.doOneFile,
            });
        }
      }
      with (dialogRows.add())
      ...
      with (borderPanels.add())
        outputDirs = dropdowns.add({
          stringList: peuINFO.outDirNameArray,
          minWidth: 204,
          selectedIndex: peuINFO.defaultDir,
        });
    }
    // Right Column 1
    with (dialogColumns.add()) {
      with (dialogRows.add()) staticTexts.add({ staticLabel: "Output Format" });
      with (borderPanels.add()) {
        with (dialogColumns.add()) {
          with (dialogRows.add()) formatTypeRB = radiobuttonGroups.add();
          with (formatTypeRB) {
            // If there are no print presets, don't add radio button for it
            if (peuINFO.adjustForNoPS == 0)
              radiobuttonControls.add({
                staticLabel: "PostScript",
                checkedState: peuINFO.exportDefaultType == 0,
              });
            radiobuttonControls.add({
              staticLabel: "PDF",
              checkedState:
                peuINFO.exportDefaultType == 1 ||
                (peuINFO.adjustForNoPS == 1 && peuINFO.exportDefaultType == 0),
            });
            radiobuttonControls.add({
              staticLabel: "EPS",
              checkedState: peuINFO.exportDefaultType == 2,
            });
            radiobuttonControls.add({
              staticLabel: "JPEG",
              checkedState: peuINFO.exportDefaultType == 3,
              minWidth: 140,
            });
            if (pseudoSingleton < 1)
              radiobuttonControls.add({
                staticLabel: "Change Preferences",
                checkedState: false,
              });
          }
        }
      ...
      if (pseudoSingleton < 2) {
        ...
        with (borderPanels.add())
          with (dialogColumns.add()) {
            with (dialogRows.add())
              doLayers = checkboxControls.add({
                staticLabel: "Layer Versioning",
                checkedState: peuINFO.layersON,
                minWidth: 140,
              });
            if (peuINFO.numDocsToExport > 1) {
              with (dialogRows.add())
                doBatch = checkboxControls.add({
                  staticLabel: "Batch Export Enabled",
                  checkedState: peuINFO.batchON,
                  minWidth: 130,
                });
              with (dialogRows.add())
                commonBatch = checkboxControls.add({
                  staticLabel: "Set For All Batch Jobs",
                  checkedState: peuINFO.batchSameForAll,
                  minWidth: 140,
                });
            }
          }
...
function usePgInfoTag(theUnits, docWidth) {
  ...
  switch (theUnits) {
    case MeasurementUnits.points:
      if (docWidth < 337.5) usePgInfo = false;
      break;
    case MeasurementUnits.picas:
      if (docWidth < 28.125) usePgInfo = false;
      break;
    case MeasurementUnits.inches:
    case MeasurementUnits.inchesDecimal:
      if (docWidth < 4.6875) usePgInfo = false;
      break;
    case MeasurementUnits.millimeters:
      if (docWidth < 119.0625) usePgInfo = false;
      break;
    case MeasurementUnits.centimeters:
      if (docWidth < 11.90625) usePgInfo = false;
      break;
    case MeasurementUnits.ciceros:
      if (docWidth < 26.3922) usePgInfo = false;
      break;
  }
  return usePgInfo;
}
...
function byeBye(msg, doTell) {
  if (peuINFO.doProgressBar) app.setProgress(999999);

  if (peuINFO.csVersion == 3)
    app.userInteractionLevel = peuINFO.oldInteractionPref;
  else app.scriptPreferences.userInteractionLevel = peuINFO.oldInteractionPref;
  if (doTell) alert(msg);
  exit();
}
...
function layerManager(theDoc) {
  ...
  do {
    getOut = true;
    var layerDialog = app.dialogs.add({
      name: 'Version Manager for "' + theDoc.name + '"',
      canCancel: true,
    });
    with (layerDialog) {
      with (dialogColumns.add()) {
        ...
        with (borderPanels.add()) {
          with (dialogColumns.add()) {
            for (lmi = 0; lmi < theLayers.length; lmi++) {
              baseControls[lmi] = checkboxControls.add({
                staticLabel: theLayers[lmi].name,
                checkedState: false,
                minWidth: 104,
              });
            }
      ...
      with (dialogColumns.add()) {
        ...
        with (borderPanels.add()) {
          with (dialogColumns.add()) {
            for (lmi = 0; lmi < theLayers.length; lmi++) {
              verControls[lmi] = checkboxControls.add({
                staticLabel: theLayers[lmi].name,
                checkedState: true,
                minWidth: 104,
              });
    ...
    if (layerDialog.show()) {
      ...
      for (lmi = 0; lmi < baseControls.length; lmi++) {
        if (
          baseControls[lmi].checkedState == true &&
          verControls[lmi].checkedState == true
        ) {
          alert(
            "A layer cannot be both a base and version layer.\nPlease try again."
          );
          getOut = false;
          break;
        }
      ...
      if (numVerChosen == 0 && getOut) {
        alert(
          "At least one version layer needs to be chosen.\nPlease try again."
        );
        getOut = false;
      }
    } else {
...
function checkPages(tempENTRY, startEndPgs) {
  ...
  if (startEndPgs == "All") {
    if (tempENTRY.doSpreadsON) {
      ...
      tempENTRY.pgEnd =
        tempENTRY.pageRangeArray[tempENTRY.pageRangeArray.length - 1];
    }

    // Tally pages for the progress bar
    tempPageCount = tempENTRY.doSpreadsON
      ? spreadCount
      : tempENTRY.theDoc.documentPreferences.pagesPerDocument;
    ...
    if (tempENTRY.doSpreadsON) {
      if (isNaN(tempENTRY.pgStart) || isNaN(tempENTRY.pgEnd)) notNumber = true;
      else if (
        tempENTRY.pgStart > spreadCount ||
        tempENTRY.pgEnd > spreadCount ||
        tempENTRY.pgStart < 0 ||
        tempENTRY.pgEnd < 0
      )
      ...
      tempENTRY.nameConvType != 4 &&
      (getPageOffset(
        tempENTRY.pgStart,
        tempENTRY.pageNameArray,
        tempENTRY.pageRangeArray
      ) == -1 ||
        getPageOffset(
          tempENTRY.pgEnd,
          tempENTRY.pageNameArray,
      ...
      (tempENTRY.pgStart < 1 ||
        tempENTRY.pgEnd < 1 ||
        tempENTRY.pgStart > tempENTRY.tempENTRY.pageRangeArray.length ||
        tempENTRY.pgEnd > tempENTRY.tempENTRY.pageRangeArray.length)
    )
      pgsNoGood = true;
    // Range OK, find out num of pages
    else
      tempPageCount =
        getPageOffset(
          tempENTRY.pgEnd,
    ...
    for (i = 0; i < tempENTRY.nonContigPgs.length; i++) {
      ...
      if (temp.indexOf("-") != -1) {
        ...
        if (tempENTRY.doSpreadsON) {
          ...
          if (isNaN(temp3) || isNaN(temp4)) notNumber = true;
          else if (
            temp3 > spreadCount ||
            temp3 < 1 ||
            temp4 > spreadCount ||
          ...
          (temp3 < 1 ||
            temp4 < 1 ||
            temp3 > tempENTRY.pageRangeArray.length ||
            temp4 > tempENTRY.pageRangeArray.length)
        )
          pgsNoGood = true;
        else if (
          (tempENTRY.nameConvType != 4 &&
          ...
          pgsNoGood = true;
        // Range OK, find out num of pages for prog bar
        else
          tempPageCount +=
            getPageOffset(
              temp4,
      ...
      // Test if a single page (was between a ",") is out of range or isn't a number
        ...
        (tempENTRY.doSpreadsON && parseInt(temp, 10) > spreadCount) ||
        (tempENTRY.doSpreadsON && parseInt(temp, 10) < 0)
      )
        spreadsNoGood = true;
      else if (
        tempENTRY.nameConvType == 4 &&
        (parseInt(temp, 10) < 1 ||
          parseInt(temp, 10) > tempENTRY.pageRangeArray.length)
      )
        pgsNoGood = true;
      else if (
        tempENTRY.nameConvType != 4 &&
    ...
    // Page range is single page (ie 5)
    tempENTRY.pgStart = startEndPgs;
    tempENTRY.pgEnd = startEndPgs;
    if (tempENTRY.doSpreadsON) {
      if (isNaN(tempENTRY.pgStart)) notNumber = true;
...
function peuInit() {
  ...
  if (File.fs != "Windows") peuINFO.previewTypes[2] = "Pict";

  // Check if preferences file exists, if not save one
  peuINFO["prefsFile"] = File(
    Folder(app.activeScript).parent + "/PageExporter5Prefs.txt"
  );
  if (!peuINFO.prefsFile.exists) savePrefs();
  else readPrefs();
...
function changePrefs() {
  ...
  if (prefItems.otherPrefsDlog.show()) {
    ...
    if (prefItems.addDefDir.checkedState) addDirectories();
    // Delete directories
    if (prefItems.delDefDir.checkedState) deleteDirectories();
    // Change Start Directory
    if (prefItems.changeStartDirectory.checkedState) {
      tempStartDirectory = getDirectory(
        "Please select the new start directory:",
        peuINFO.startingDirectory
      );
    ...
    peuINFO.baseLaersAsVersion = prefItems.changeBaseExport.checkedState
      ? 1
      : 0;
...
function makePrefsDlog() {
  ...
  prefsItems["otherPrefsDlog"] = app.dialogs.add({
  ...
  with (prefsItems.otherPrefsDlog) {
    ...
    with (dialogColumns.add()) {
      ...
      with (borderPanels.add()) {
        with (dialogColumns.add()) {
          with (dialogRows.add())
            prefsItems["addDefDir"] = checkboxControls.add({
              staticLabel: "Add an Output Directory",
              checkedState: false,
            });
          with (dialogRows.add())
            prefsItems["delDefDir"] = checkboxControls.add({
              staticLabel: "Delete an Output Directory",
              checkedState: false,
            });
          with (dialogRows.add())
            prefsItems["changeStartDirectory"] = checkboxControls.add({
              staticLabel: "Change Starting Directory",
              checkedState: false,
            });
          with (dialogRows.add()) {
          ...
          with (dialogRows.add()) {
            ...
            prefsItems["changeUnits"] = dropdowns.add({
              stringList: peuINFO.measureLableArray,
              selectedIndex: peuINFO.measurementUnits,
            });
          }
          with (dialogRows.add()) {
            staticTexts.add({ staticLabel: "Add Leading Zero(s)" });
            prefsItems["changeAddZero"] = dropdowns.add({
              stringList: ["No", "Yes"],
              selectedIndex: peuINFO.addZeroON,
            });
          }
          with (dialogRows.add()) {
            staticTexts.add({ staticLabel: "Force Page Numbers to" });
            prefsItems["changePadding"] = dropdowns.add({
              stringList: ["Normal", "2", "3"],
              selectedIndex: peuINFO.zeroPadding,
            });
            staticTexts.add({
              staticLabel: "Digits when Adding Leading Zeros",
            });
          }
          with (dialogRows.add()) {
            staticTexts.add({ staticLabel: 'Add "P" to Numeric Pages' });
            prefsItems["changeAddP"] = dropdowns.add({
              stringList: ["No", "Yes"],
              selectedIndex: peuINFO.addPon,
            });
          }
          with (dialogRows.add()) {
            staticTexts.add({ staticLabel: "Add Page Name" });
            prefsItems["changeBeforeON"] = dropdowns.add({
              stringList: ["After", "Before", 'by replacing "<#>" in'],
              selectedIndex: peuINFO.pageNamePlacement,
            });
            staticTexts.add({ staticLabel: "Base Name" });
          }
          with (dialogRows.add()) {
            staticTexts.add({ staticLabel: "Add Layer Name" });
            prefsItems["changeLayerON"] = dropdowns.add({
              stringList: ["After", "Before"],
              selectedIndex: peuINFO.layerBeforeON,
            });
            staticTexts.add({ staticLabel: "Page Name" });
          }
          with (dialogRows.add()) {
            staticTexts.add({ staticLabel: "Alert on Cancel" });
            prefsItems["changeShowCancel"] = dropdowns.add({
              stringList: ["No", "Yes"],
              selectedIndex: peuINFO.sayCancel,
            });
          }
          with (dialogRows.add()) {
            staticTexts.add({ staticLabel: "Lot Naming Type" });
            prefsItems["changeSubType"] = dropdowns.add({
              stringList: ["Consecutive", "Redundant"],
              selectedIndex: peuINFO.subType,
            });
          }
          with (dialogRows.add())
            prefsItems["changeBaseExport"] = checkboxControls.add({
              staticLabel: "Export Base Layer(s) as an Individual Version",
              checkedState: peuINFO.baseLaersAsVersion,
              minWidth: 140,
            });
        }
      }
      ...
      do {
        ...
        with (tempDlog)
          with (dialogColumns.add())
            with (dialogRows.add()) {
              staticTexts.add({
                staticLabel: "Enter the name to use for this new directory:",
              });
              var tempChange = textEditboxes.add({
                editContents: "newName",
                minWidth: 170,
              });
            }
        if (tempDlog.show()) {
          ...
          } else peuINFO.outDirNameArray.push(tempChange);
        }
      } while (!prefGetOut);
    }
    if (prefGetOut1) {
      ...
      with (tempDlog)
        with (dialogColumns.add())
          with (dialogRows.add())
            staticTexts.add({
              staticLabel:
                "Do you want to add another default directory? (OK=yes, Cancel=no)",
            });
...
function deleteDirectories() {
  do {
    ...
    var tempDlog = app.dialogs.add({
      name: "Delete an Output Directory",
      canCancel: true,
    });
    with (tempDlog)
      with (dialogColumns.add())
        with (dialogRows.add())
          var killIt = dropdowns.add({
            stringList: menuArray,
            minWidth: 204,
            selectedIndex: 0,
          });
    if (tempDlog.show()) {
      killIt = killIt.selectedIndex;
      if (killIt == peuINFO.defaultDir) peuINFO.defaultDir--;
      if (killIt == 0)
    ...
    with (tempDlog)
      with (dialogColumns.add())
        with (dialogRows.add())
          staticTexts.add({
            staticLabel:
              "Do you want to delete another default directory? (OK=yes, Cancel=no)",
          });
